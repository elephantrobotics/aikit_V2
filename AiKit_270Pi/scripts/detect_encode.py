...
class Detect_marker():
    ...
    def move(self, x, y):
        ...
        time.sleep(3.5)
        self.mc.send_coords([coords[0][0]+x, coords[0][1]+y, 108, 172.36, 5.36, 125.58], 30, 0)
        time.sleep(3.5)
        self.pub_pump(True)
        time.sleep(2.5)
        self.mc.send_coords(coords[0], 30, 0)
        time.sleep(4)
        self.mc.send_coords(coords[1], 30, 0)
        time.sleep(4)
        self.pub_pump(False)  
        time.sleep(5)
        self.mc.send_coords(coords[0], 30, 0)
    ...
    def decide_move(self, x, y):
        ...
        if (abs(x - self.cache_x) + abs(y - self.cache_y)) / 2 > 5: # mm
            ...
            self.move(x+38, y+138)

    # init mycobot
    def init_mycobot(self):
        self.pub_pump(False)
        
        self.mc.send_coords([81.8, -52.3, 186.7, 174.48, 4.08, 92.41], 20, 0)
        time.sleep(2)
